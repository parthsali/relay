import { Cpu, Thermometer, Zap, Wifi, Activity } from "lucide-react";

const metrics = [
  { label: "CPU Usage", value: "8%", max: 8, status: "good", icon: Cpu },
  { label: "RAM Usage", value: "19%", max: 19, status: "good", icon: Zap },
  { label: "Temperature", value: "46°C", max: 58, status: "good", icon: Thermometer },
  { label: "Wi-Fi Signal", value: "−62 dBm", max: 78, status: "good", icon: Wifi },
];
  ...
  return (
    <div className="flex w-full flex-1 flex-col">
      <div className="border-b border-border px-8 py-6">
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">Real-time system performance metrics.</p>
      </div>
      ...
      <div className="flex flex-col gap-6 px-8 py-6">
            ...
            return (
              <div key={m.label} className="rounded-lg border border-border p-4">
                ...
                <div className="mt-3 h-1 overflow-hidden rounded-full bg-muted">
                  <div className={`h-full rounded-full ${statusColor[m.status]}`} style={{ width: `${m.max}%` }} />
                </div>
              </div>
            );
          })}
        ...
        {/* Status checks */}
        <div>
          <p className="mb-3 text-sm font-medium text-muted-foreground">Status checks</p>
          <div className="overflow-hidden rounded-lg border border-border">
            {[
              ...
              <div key={i} className="flex items-center gap-3 border-b border-border px-4 py-3 last:border-0">
                <span className={`size-1.5 rounded-full ${c.ok ? "bg-emerald-500" : "bg-red-500"}`} />
                <span className="flex-1 text-sm">{c.check}</span>
                <span className={`text-xs font-medium ${c.ok ? "text-emerald-500" : "text-red-500"}`}>
                  {c.ok ? "Pass" : "Fail"}
                </span>
              </div>
            ))}
