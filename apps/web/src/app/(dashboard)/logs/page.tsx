...
const logs = [
  ...
  { ts: "10:50:01", level: "INFO", msg: "Weather data refreshed successfully" },
  {
    ts: "10:45:22",
    level: "INFO",
    msg: "Spotify track changed: Blinding Lights → Starboy",
  },
  ...
  { ts: "10:41:55", level: "INFO", msg: "Agent connected · relay-pi.local" },
  {
    ts: "10:41:50",
    level: "DEBUG",
    msg: "JWT verified for user parth@example.com",
  },
  { ts: "10:40:00", level: "WARN", msg: "CPU temperature approaching 70°C" },
  {
    ts: "10:38:12",
    level: "INFO",
    msg: "Scheduled task triggered: Work Hours Stats",
  },
  {
    ts: "10:30:00",
    level: "ERROR",
    msg: "Failed to reach weather API — retrying in 60s",
  },
];
        ...
        <div className="flex-1 overflow-auto rounded-lg border border-border bg-muted/30 p-4 font-mono text-xs leading-relaxed">
          {logs.map((log, i) => (
            <div key={i} className="flex gap-4 py-0.5">
              ...
              <span
                className={`w-12 shrink-0 font-semibold ${levelColor[log.level]}`}
              >
                {log.level}
              </span>
