...
import {
  Music2, Clock, Cloud, ImageIcon, Film, Type, CalendarDays,
  BarChart2, Monitor, SlidersHorizontal, Layers, Play,
} from "lucide-react";
...
function SpotifyConfig() {
  return (
    <div className="flex flex-col gap-4">
      {[
        ["Enable", true], ["Show Album Art", true], ["Show Song Name", true],
        ["Show Artist", true], ["Progress Bar", true], ["Background Blur", false],
...
function WeatherConfig() {
  return (
    <div className="flex flex-col gap-4">
      {[
        ["Show Humidity", true], ["Show Wind", true],
        ["Show Forecast", false], ["Weather Icons", true],
      ].map(([label, def]) => (
        <div key={label as string} className="flex items-center justify-between">
          <Label className="text-sm text-muted-foreground">{label as string}</Label>
          <Switch defaultChecked={def as boolean} />
      ...
      <div className="flex w-56 shrink-0 flex-col border-r border-border overflow-y-auto px-2 py-4">
        ...
        {modes.map((m) => {
          ...
          return (
            ...
            >
              <Icon className={cn("size-3.75 shrink-0", active === m.id ? "text-foreground" : "text-muted-foreground/50")} />
              {m.label}
            </button>
          );
        })}
      ...
      <div className="flex flex-1 flex-col gap-8 px-10 py-9 overflow-y-auto">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[13px] text-muted-foreground">Configure display settings</p>
          </div>
          <Button size="sm">
